...
const Achievements = () => {
    ...
    const fetchData = async () => {
      ...
      if (session) {
        ...
        const [certRes, earnedAchRes, allAchRes, enrollmentRes] = await Promise.all([
          ...
          supabase
            .from('certificates')
            .select('id, course_id, issued_at, certificate_url, courses(title, category, image_url, instructor_id, instructor)')
            .eq('user_id', uid)
        ...
        // Fetch instructor profiles for these certificates
        const instructorIds = [...new Set(dbCerts.map(c => c.courses?.instructor_id).filter(Boolean))];
        let instructorsMap = {};
        ...
        // Use only officially issued certificates from the DB, decorated with instructor info
        const mergedCerts = dbCerts.map(c => {
          const instructor = c.courses?.instructor_id ? instructorsMap[c.courses.instructor_id] : null;
          return {
            ...c,
            instructor
          };
  ...
  const handlePrintCertificate = (cert) => {
    ...
    const teacherName = cert.instructor?.full_name || cert.courses?.instructor || "Harin AI System";
    const signatureImg = cert.instructor?.signature_url ? `<img src="${cert.instructor.signature_url}" style="height:60px; max-width:150px; object-fit:contain; margin-bottom:-10px;" />` : `<div style="height:50px;"></div>`;
  ...
  return (
    <div className="bg-surface text-on-surface font-body-md flex h-screen overflow-hidden">
      ...
      <main className="flex-1 h-full overflow-y-auto bg-background p-4 md:p-8 lg:p-10 pb-24 md:pb-8">
        {/* Header */}
                  ...
                  <div className="w-full max-w-2xl border-4 border-on-surface bg-[#fffcf5] p-6 md:p-10 text-center relative shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] font-sans" style={{ borderStyle: 'double', borderWidth: '10px' }}>
                    ...
                    <div className="flex justify-between items-end mt-4 md:mt-8 px-2 text-left">
                      ...
                      <div className="text-center flex flex-col items-center">
                        {selectedCertForPreview.instructor?.signature_url ? (
                          <img 
                            src={selectedCertForPreview.instructor.signature_url} 
                            alt="Signature" 
                            className="h-10 md:h-14 max-w-[120px] object-contain mb-[-8px]"
                          />
                        ) : (
                        ...
                        <div className="text-[8px] md:text-[10px] font-black uppercase text-on-surface">
                          {selectedCertForPreview.instructor?.full_name || selectedCertForPreview.courses?.instructor || "Harin AI System"}
                        </div>
